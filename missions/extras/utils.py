...
def generate_report_or_attachments(mission_id, zip_attachments=False):
    ...
    total_tests_with_findings = TestDetail.objects.filter(
        mission=mission, has_findings=True
    ).count()
    total_tests_without_findings = TestDetail.objects.filter(
        mission=mission, has_findings=False
    ).count()
    ...
    def add_to_data_dir(mission_data_dir, test_case_number, supporting_data):
        ...
        shutil.copy(os.path.join(settings.MEDIA_ROOT, supporting_data.filename()), path)
    ...
    for t in tests:
        ...
        is_last_test_case = (
            True if test_case_number == total_reportable_tests else False
        )
        if t.has_findings:
            ...
            if (
                tests_without_findings == total_tests_without_findings
                or is_last_test_case
            ):
        ...
        if mission.test_case_identifier:
            get_cleared_paragraph(cell).text = "Test #{0}-{1}".format(
                mission.test_case_identifier, test_case_number
            )
        else:
        ...
        if (
            mission.attack_phase_include_flag
        ...
        if (
            mission.attack_type_include_flag
        ...
        if mission.test_description_include_flag and t.test_description_include_flag:
            ...
            if t.re_eval_test_case_number:
                cell.text = standardize_report_output_field(
                    "This is a reevaluation; reference previous test case #{0}\n\n{1}".format(
                        t.re_eval_test_case_number, t.test_description
                    )
                )
            else:
        ...
        row_number += 1
        if (
            mission.attack_side_effects_include_flag
            and t.attack_side_effects_include_flag
        ):
            cell = get_cleared_paragraph(table.cell(row_number, 1))
        ...
        row_number += 1
        if (
            mission.test_result_observation_include_flag
            and t.test_result_observation_include_flag
        ):
            cell = get_cleared_paragraph(table.cell(row_number, 1))
        ...
        if mission.supporting_data_include_flag:
            my_data = TestSortingHelper.get_ordered_supporting_data(
                test_detail_id=t.id, reportable_supporting_data_only=True
            )
            ...
            if len(my_data) > 0:
                ...
                for d in my_data:
                    ...
                    try:
                        ...
                        get_cleared_paragraph(content_cell).add_run().add_picture(
                        ...
                        logger.debug(
                            ">> Attachment {attachment_name} not recognized as an image; adding as file.".format(
                        ...
                        logger.warning(
                            ...
                            "\n{trace}".format(
                                file_name=d.filename, trace=traceback.format_exc(10)
                            )
                        )
                    except OSError:
                        logger.warning(
