...
class TestSortingHelper(object):
    ...
    def deconflict_and_update(mission_id, tests=None):
        ...
        logger.debug(
            "Performing TC deconflict and update (mission {mission})".format(
                mission=mission_id
            )
        )
        ...
        if tests and not sort_order:
            logger.debug(
                "TC Deconflict found TCs but no existing order (mission {mission}); "
                + "Building sort order from test case numbers".format()
            )
        ...
        logger.debug(
            "TC Deconflict is reconciling sort order with the database (mission {mission})".format(
                mission=mission_id
            )
        )
        ...
        for x in sort_order:
            if x in test_ids_from_db:
                ...
                logger.info(
                    "Sort order is dirty (TC has been deleted from DB) (mission {mission}, tc {tc})".format(
                        mission=mission_id, tc=x
                    )
                )
        ...
        for x in test_ids_from_db:
            if x not in deconflicted_sort_order:
                deconflicted_sort_order.append(x)
                logger.info(
                    "Sort order is dirty (TC has been added to DB) (mission {mission}, tc {tc})".format(
                        mission=mission_id, tc=x
                    )
                )
        ...
        if sort_order_dirty:
            ...
            logger.info(
                "Reconciliation Performed (Mission %s): "
                "Mission Sort Order: %s; "
                "Database TC Records: %s; "
                "Result of Deconflict: %s"
                % (mission_id, sort_order, test_ids_from_db, deconflicted_sort_order)
            )
    ...
    def deconflict_and_update_supporting_data_sort_order(testdetail_id, testdata=None):
        ...
        logger.debug(
            "Performing Supporting Data deconflict and update (testdetail {testdetail})".format(
                testdetail=testdetail_id
            )
        )
        ...
        if testdata and not sort_order:
            logger.debug(
                "Supporting data for test found but no existing order (testdetail {testdetail}); "
                + "Building default sort order".format()
            )
            sort_order = [data.id for data in testdata]

        # Perform a reconcile to catch edge cases where testdata have been added or deleted
        logger.debug(
            "Supporting Data Deconflict is reconciling sort order with the database (testdetail {testdetail})".format(
                testdetail=testdetail_id
            )
        )
        ...
        for x in sort_order:
            if x in data_ids_from_db:
                ...
                logger.info(
                    "Sort order is dirty (Supporting data has been deleted from DB) "
                    "(testdetail {testdetail}, sd {sd})".format(
                        testdetail=testdetail_id, sd=x
                    )
                )
                sort_order_dirty = True  # Id in mission's sort order, but not database (it's been deleted)
        ...
        for x in data_ids_from_db:
            if x not in deconflicted_sort_order:
                deconflicted_sort_order.append(x)
                logger.info(
                    "Sort order is dirty (Supporting data has been added to DB) "
                    "(testdetail {testdetail}, sd {sd})".format(
                        testdetail=testdetail_id, sd=x
                    )
                )
        ...
        # Save the reconciled sort order
        if sort_order_dirty:
            testdetail_model.supporting_data_sort_order = json.dumps(
                deconflicted_sort_order
            )
            testdetail_model.save(update_fields=["supporting_data_sort_order"])
            logger.info(
                "Reconciliation Performed (TestDetail %s): "
                "TestDetail Supporting Data Sort Order: %s; "
                "Suppporting Data from Database: %s; "
                "Result of Deconflict: %s"
                % (testdetail_id, sort_order, data_ids_from_db, deconflicted_sort_order)
            )
    ...
    def get_ordered_testdetails(cls, mission_id, reportable_tests_only=False):
        ...
        if reportable_tests_only:
            excluded_tests = tests.filter(test_case_include_flag=False).values_list(
                "id", flat=True
            )
    ...
    @classmethod
    def get_ordered_supporting_data(
        cls, test_detail_id, reportable_supporting_data_only=False
    ):
        try:
        ...
        sort_order = cls.deconflict_and_update_supporting_data_sort_order(
            test_detail_id, testdata
        )

        # Order
        excluded_data = []
        if reportable_supporting_data_only:
            excluded_data = testdata.filter(include_flag=False).values_list(
                "id", flat=True
            )
