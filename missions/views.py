...
class UpdateDynamicSettingsView(UpdateView):
    ...
    fields = ["system_classification", "host_output_format"]
...
class EditMissionView(UpdateView):
    ...
    def get_success_url(self):
        logger.debug(
            "POST: EditMissionView (Saved {mission_id})".format(
    ...
    def get_context_data(self, **kwargs):
        logger.debug(
            "GET: EditMissionView (Edit {mission_id})".format(
                mission_id=self.get_object().id
            )
        )
        context = super(EditMissionView, self).get_context_data(**kwargs)
        context["action"] = reverse(
            "missions-edit", kwargs={"pk": self.get_object().id}
        )
        return context
...
class DeleteMissionView(DeleteView):
    ...
    def get_context_data(self, **kwargs):
        logger.debug(
            "GET: DeleteMissionView (Confirm delete {mission_id})".format(
                mission_id=self.object.id
            )
        )
    ...
    def get_success_url(self):
        logger.debug(
            "POST: DeleteMisisonView (Deleted {mission_id})".format(
                mission_id=self.object.id
            )
        )
        return reverse("missions-list")
...
class ReportMissionView(View):
    def get(self, request, *args, **kwargs):
        ...
        logger.debug(
            "GET: ReportMissionView ({mission_id})".format(mission_id=mission_id)
        )

        io_stream, name = generate_report_or_attachments(
            mission_id, zip_attachments=False
        )
        docx_name = name + "_" + str(mission_id)

        response = HttpResponse(io_stream.getvalue(), content_type="text/plain")
        response["Content-Disposition"] = (
            "attachment; filename={}_mission_report.docx".format(docx_name)
        )
...
class ReportAttachmentsMissionView(View):
    def get(self, request, *args, **kwargs):
        ...
        logger.debug(
            "GET: ReportAttachmentsMissionView ({mission_id})".format(
                mission_id=mission_id
            )
        )

        io_stream, name = generate_report_or_attachments(
            mission_id, zip_attachments=True
        )
        zip_name = name + "_" + str(mission_id)

        response = HttpResponse(
            io_stream.getvalue(), content_type="application/octet-stream"
        )
        response["Content-Disposition"] = (
            "attachment; filename={}_supporting_data.zip".format(zip_name)
        )
        return response
...
class CloneMissionTestView(View):
    def get(self, request, *args, **kwargs):
        ...
        if test_case.mission.id != passed_mission_id:
            return HttpResponse(
                "Test case not linked to specified mission.", status=400
            )
...
class CreateMissionTestView(CreateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["action"] = reverse(
            "mission-test-new", kwargs={"mission": self.kwargs["mission"]}
        )
        context["this_mission"] = Mission.objects.get(id=self.kwargs["mission"])
...
class EditMissionTestView(UpdateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["action"] = reverse(
            "mission-test-edit",
            kwargs={"pk": self.get_object().id, "mission": self.kwargs["mission"]},
        )
        ...
        context["is_read_only"] = (
            resolve(self.request.path_info).url_name == "mission-test-view"
        )
        if self.request.GET.get("scrollPos"):
            try:
                ...
                logger.exception(
                    "URL Parameter scrollPos invalid (not an int); setting to None. "
                    "(Logged in user: {user})".format(
...
class ListMissionTestsSupportingDataView(ListView):
    ...
    def get_context_data(self, **kwargs):
        context = super(ListMissionTestsSupportingDataView, self).get_context_data(
            **kwargs
        )
...
class CreateMissionTestsSupportingDataView(CreateView):
    ...
    def get_context_data(self, **kwargs):
        context = super(CreateMissionTestsSupportingDataView, self).get_context_data(
            **kwargs
        )
        context["action"] = reverse(
            "test-data-new",
...
class EditMissionTestsSupportingDataView(UpdateView):
    ...
    def get_context_data(self, **kwargs):
        context = super(EditMissionTestsSupportingDataView, self).get_context_data(
            **kwargs
        )
        context["action"] = reverse(
            "test-data-edit",
...
class DeleteMissionTestsSupportingDataView(DeleteView):
    ...
    def get_context_data(self, **kwargs):
        context = super(DeleteMissionTestsSupportingDataView, self).get_context_data(
            **kwargs
        )
...
class OrderMissionTestsSupportingDataView(View):
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            logger.exception(
                "POST: OrderMissionTestsSupportingDataView received non-ints"
            )
            raise
...
class DownloadSupportingDataView(View):
    ...
    def get(self, request, *args, **kwargs):
        supporting_data_object = SupportingData.objects.get(
            id=self.kwargs["supportingdata"]
        )

        # Backwards compatibility shim for #106 fix
        if str(supporting_data_object.test_file.name).startswith("supporting_data/"):
            supporting_data_object.test_file.name = (
                supporting_data_object.test_file.name[16:]
            )
            supporting_data_object.save()

        filename = supporting_data_object.filename()
        response = HttpResponse(
            supporting_data_object.test_file.file, content_type="text/plain"
        )
...
class LoginInterstitialView(TemplateView):
    ...
    def post(self, request):
        ...
        logger.info(
            "User ({user}) acknowledged and accepted the login interstitial".format(
...
class EditMissionHostsView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["mission"] = Mission.objects.prefetch_related("host_set").get(
            pk=int(kwargs.get("mission_id"))
        )
        return context
        ...
        try:
            if len(request.body) > 0:
                ...
                try:
                    ...
                    logger.debug(
                        "Host update POST pk value does not correspond to a host in the host_set."
                    )
                ...
                if host.pk is None:
                    ...
                    if "mission_id" in list(data.keys()):
                        ...
                        try:
                            host.mission = Mission.objects.get(
                                pk=int(data["mission_id"])
                            )
                        except Mission.DoesNotExist:
                            error_message = "Unable to find mission."
                            logger.debug(error_message)
                            return JsonResponse(
                                ReturnStatus(False, error_message).to_dict()
                            )
                    else:
                        error_message = (
                            "Host requires an associated mission (not provided)."
                        )
                        logger.debug(error_message)
                        return JsonResponse(
                            ReturnStatus(False, error_message).to_dict()
                ...
                try:
                    ...
                    return JsonResponse(
                        ReturnStatus(message="OK", data={"pk": host.pk}).to_dict()
                    )
                except KeyError:
                    error_message = (
                        "Required field(s) missing from host update / creation."
                    )
                    logger.debug(error_message)
                    return JsonResponse(
                        ReturnStatus(success=False, message=error_message).to_dict()
                    )
    ...
    if request.method == "DELETE":
        try:
            ...
            if len(combined_set) > 0:
                ...
                error_message = (
                    "Host {host} is currently listed as a source or target on one or more test cases. "
                ...
                host.delete()
                return JsonResponse(
                    ReturnStatus(message="OK", data={"pk": host_id}).to_dict()
                )
        except Host.DoesNotExist:
            ...
            logger.debug(error_message)
            return JsonResponse(
                ReturnStatus(False, error_message).to_dict(), status=BAD_REQUEST
            )
...
def test_host_handler(request, mission_id, test_id):
    if request.method == "GET":
        try:
            testcase = (
                TestDetail.objects.prefetch_related("target_hosts")
                .prefetch_related("source_hosts")
                .get(pk=test_id)
            )
    ...
    if request.method == "POST":
        try:
            if len(request.body) > 0:
                ...
                logger.debug(
                    "test_host_handler POST added host {} to testcase {}".format(
                        host.pk,
                        testcase.pk,
                    )
                )
            ...
            logger.debug(
                "test_host_handler POST encountered an error decoding the JSON input"
            )
        ...
        try:
            ...
            if ba.mission_set.all().count() != 0:
                return JsonResponse(
                    ReturnStatus(
                        False,
                        "Business Areas can not be deleted while missions are still associated with them.",
                    ).to_dict()
                )
            ba.delete()
        except BusinessArea.DoesNotExist:
    ...
    if request.method == "POST":
        ...
        if pk is None:
            ...
            try:
                ...
                return JsonResponse(
                    ReturnStatus(
                        False, "One or more of the selected colors does not exist."
                    ).to_dict()
                )
            return JsonResponse(ReturnStatus(True, "OK").to_dict())
        else:
            ...
            try:
                ...
                classification.background_color = Color.objects.get(
                    pk=data["background_color"]
                )
                classification.report_label_color_selection = data[
        ...
        try:
            ...
            if (
                classification
                == DARTDynamicSettings.objects.get_as_object().system_classification
            ):
                return JsonResponse(
                    ReturnStatus(
                        False,
                        "Can not delete the classification the system is currently operating at. Change system classification and try again.",
                    ).to_dict()
                )
        ...
        try:
            ...
            if color == active_text_color or color == active_background_color:
                return JsonResponse(
                    ReturnStatus(
                        False,
                        "Can not delete a color in use by the classification legend the system is currently operating at. Change system classification or color options and try again.",
                    ).to_dict()
                )
            color.delete()
